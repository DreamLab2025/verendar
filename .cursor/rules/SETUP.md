# 🚀 SETUP PROJECT CHO HAU - VERENDAR

Hướng dẫn setup từ đầu project Next.js 16 cho **Verendar** (setup project cho Hau) - ứng dụng quản lý và nhắc bảo dưỡng phương tiện cá nhân, tích hợp bản đồ để điều hướng người dùng đến garage, sử dụng Redux Toolkit, TanStack React Query, **Tailwind CSS v4**, SignalR real-time, RBAC Middleware.

## CONTEXT DU AN VERENDAR (DOC TU README)

- Frontend cho hệ thống quản lý và nhắc bảo dưỡng phương tiện với Next.js App Router.
- Tính năng chính: auth đầy đủ (login/register/forgot/OTP/onboarding), quản lý xe, lịch sử công-tơ-mét, quản lý bảo dưỡng, thông báo realtime SignalR.
- Có khu vực admin riêng: quản lý users, brands, models, variants, parts, products, vehicles.
- Có tích hợp Google Maps cho màn hình map và điều hướng garage.
- Route groups chính trong `app/`: `(auth)`, `(user)`, `(vehicle)`, `(admin)`, `(notifications)`.
...
NEXT_PUBLIC_APP_NAME=Verendar
NEXT_PUBLIC_APP_URL=http://localhost:3000
NEXT_PUBLIC_CRYPTO_SECRET_KEY=your-secret-key-here
NEXT_PUBLIC_ENV=development
```

> ⚠️ **Production**: Set `NEXT_PUBLIC_ENV=production`, `NEXT_PUBLIC_API_URL_BACKEND` trỏ về backend và `NEXT_PUBLIC_API_URL_API_GATEWAY` trỏ về API gateway/notification hub.
...
@layer base {
  :root {
    /* Brand Colors - Verendar (chu dao do) */
    --brand-primary: #cd2626;
    --brand-primary-dark: #a81f1f;
...
@theme {
  ...
  --color-brand-neutral-dark: var(--brand-neutral-dark);
  --color-brand-neutral-light: var(--brand-neutral-light);
        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
          ...
          try {
            ...
            const response = await axios.post(
              `${process.env.NEXT_PUBLIC_API_URL_BACKEND}api/v1/auth/refresh-token`,
              { refreshToken },
              { headers: { "Content-Type": "application/json" } }
            );
...
function getBaseUrl(): string {
  return process.env.NEXT_PUBLIC_API_URL_API_GATEWAY || "http://localhost:8080/";
}
...
export const metadata: Metadata = {
  ...
  description: 'Verendar la app quan ly xe ca nhan, nhac nho thay the phu tung va dieu huong den garage gan nhat qua map.',
  keywords: ['verendar', 'quan ly xe ca nhan', 'nhac nho thay phu tung', 'garage map'],
  openGraph: {
    type: 'website',
    locale: 'vi_VN',
    url: process.env.NEXT_PUBLIC_APP_URL,
    title: 'Verendar',
    description: 'Quan ly xe ca nhan, nhac lich bao tri va dieu huong garage tren ban do.',
    siteName: 'Verendar',
  },
...
export function middleware(request: NextRequest) {
  ...
  const isVehicleProfileRoute = pathname.startsWith("/my-vehicles");
  ...
  if (hasRole(userRoles, "ROLE_ADMIN")) {
    ...
    if (userRoles.length > 1) {
      ...
      if ((isCoursesRoute || isVehicleProfileRoute) && hasRole(userRoles, "ROLE_STUDENT"))
        return NextResponse.next();
  ...
  if (hasRole(userRoles, "ROLE_INSTRUCTOR")) {
    ...
    if (isVehicleProfileRoute) {
      ...
      return NextResponse.redirect(new URL("/my-vehicles?tab=myprofile", request.url));
    }
...
5. **RBAC Middleware** — jwtDecode, role-based routing, per-tab vehicle profile control
6. **Real-time** — SignalR hub qua API gateway, SignalRProvider, notification hooks, cross-tab auth sync
...
**Setup này đã được chỉnh cho đúng bối cảnh Verendar theo README: quản lý phương tiện, nhắc bảo dưỡng, thông báo realtime và map điều hướng garage.**
